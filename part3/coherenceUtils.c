...
enum state determineState(cache_t* cache, uint32_t address) {
	...
	uint32_t block = info->blockNumber;
	uint8_t valid = getValid(cache,block);
	uint8_t dirty = getDirty(cache,block);
	uint8_t shared = getShared(cache,block);
	enum state ret = INVALID;
	if (info->match == 0) {
        	free(info);
		return INVALID;
	}
	free(info);
	if (!valid && !dirty && !shared) return INVALID;
	else if (!valid && !dirty && shared) return INVALID;
	else if (!valid && dirty && !shared) return INVALID;
	else if (!valid && !dirty && shared) return INVALID;
	else if (valid && !dirty && !shared) return EXCLUSIVE;	
	else if (valid && !dirty && shared) return SHARED;
	else if (valid && dirty && !shared) return MODIFIED;
	else if (valid && dirty && shared) return OWNED;
...
void setState(cache_t* cache, uint32_t blockNumber, enum state newState) {
	...
	if (newState == INVALID) {
		...
		setLRU(cache,blockNumber,0xffff);
	}
...
void updateState(cache_t* cache, uint32_t address, enum state otherState) {
	/* Your Code Here. */
        enum state  ownState = determineState(cache,address); 
        evictionInfo_t* info = findEviction(cache, address);
        uint32_t oldLRU = getLRU(cache, info->blockNumber);
        uint32_t idx = getIndex(cache,address);
        uint32_t tag = getTag(cache,address) ;

        if (otherState == SHARED && ownState == EXCLUSIVE){
        	setState(cache,info->blockNumber,SHARED);
        }
        if (otherState == SHARED && ownState == MODIFIED){
                setState(cache,info->blockNumber,OWNED);
        }
        if (otherState == SHARED && ownState == INVALID){
                setState(cache,info->blockNumber,SHARED);
        }
	if (otherState == SHARED && ownState == OWNED){
                setState(cache,info->blockNumber,SHARED);
        }
        if (otherState == INVALID && ownState == SHARED){
        	setState(cache,info->blockNumber,EXCLUSIVE);
        }
        if (otherState == INVALID && ownState == OWNED){
        	setState(cache,info->blockNumber,MODIFIED);
        }
        if (otherState == MODIFIED && ownState == OWNED){
        	setState(cache,info->blockNumber,INVALID);
        	decrementLRU(cache, tag,idx, oldLRU);
        }
        if (otherState == MODIFIED && ownState == SHARED){
        	setState(cache,info->blockNumber,INVALID);
        	decrementLRU(cache, tag,idx, oldLRU);
        }
        if (otherState == MODIFIED && ownState == EXCLUSIVE){
                setState(cache,info->blockNumber,INVALID);
                decrementLRU(cache, tag,idx, oldLRU);
        }
        if (otherState == MODIFIED && ownState == MODIFIED){
                setState(cache,info->blockNumber,INVALID);
                decrementLRU(cache, tag,idx, oldLRU);
        }
	if (otherState == EXCLUSIVE && ownState == SHARED){
                setState(cache,info->blockNumber,EXCLUSIVE);
