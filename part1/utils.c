...
/*
	Accepts a pointer and checks for null when mallocing
*/
static void wrap_malloc(void* p) {
  if (!p) {
/*    allocation_failed();*/
  }
}
...
cache_t* createCache(uint32_t n, uint32_t blockDataSize, uint32_t totalDataSize, char* physicalMemoryName) {
	...
	/* Your Code Here. */
	if ( n == 0 || blockDataSize == 0 || totalDataSize == 0 || physicalMemoryName == NULL){
		invalidCache();
		return NULL;
	}
	...
	cache->n = n;
	cache->blockDataSize = blockDataSize;
	cache->totalDataSize = totalDataSize;
	cache->access = 0.0;
	cache->hit = 0.0;

	cache->physicalMemoryName = (char*) malloc(strlen(physicalMemoryName)+1);
	wrap_malloc(cache->physicalMemoryName);
	cache->contents = (uint8_t*) malloc(sizeof(uint8_t) * cacheSizeBytes(cache));
	wrap_malloc(cache->contents);
	strcpy(cache->physicalMemoryName, physicalMemoryName);
	clearCache(cache);
	return cache;
...
uint32_t getIndex(cache_t* cache, uint32_t address) {
	/* Your Code Here. */
	uint8_t index = log_2(cache->totalDataSize/(cache->blockDataSize*cache->n));
	if (index == 0) { return 0;}
	address = address << (32 - (index + log_2(cache->blockDataSize)));
	return address >> (32  - index);
}
...
uint8_t getTagSize(cache_t* cache) {
	/* Your Code Here. */
	/*SHOULD WE CHECK FOR DIRTY BITS AND LRU AS PART OF THE ADDRESS SIZE*/
	uint8_t index = log_2(cache->totalDataSize/(cache->blockDataSize*cache->n));
	uint8_t offset = log_2(cache->blockDataSize);
...
uint64_t totalBlockBits(cache_t* cache) {
	/* Your Code Here. */
	return (uint64_t) cache->blockDataSize * 8 + (uint64_t) getTagSize(cache) + (uint64_t) log_2(cache->n) + 3;
...
uint64_t getBlockStartBits(cache_t* cache, uint32_t blocknumber) {
	/* Your Code Here. */
	uint64_t index;
	uint64_t garbage = (uint64_t) numGarbageBits(cache);
	index = garbage + (uint64_t) blocknumber * totalBlockBits(cache);
	return index;
}
...
uint64_t getSharedLocation(cache_t* cache, uint32_t blockNumber) {
	/* Your Code Here. */
	return getValidLocation(cache,blockNumber) + 2;
}
...
uint64_t getLRULocation(cache_t* cache, uint32_t blockNumber) {
	/* Your Code Here. */
	return getValidLocation(cache,blockNumber) + 3;
}
...
uint64_t getTagLocation(cache_t* cache, uint32_t blockNumber) {
	/* Your Code Here. */
	return getLRULocation(cache,blockNumber) + (uint64_t) log_2(cache->n);
}
